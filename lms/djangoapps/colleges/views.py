...
def get_college_courses(courses):
    num_list = []
    school_list = []
    for course in courses:
        org = course.display_org_with_default
        if org not in school_list:
            school_list.append(org)

    num=0
    for school in school_list:
        for course in courses:
            if course.display_org_with_default == school:
                num += 1

        num_list.append((num, school))
        num = 0

    list = sorted(num_list, key=lambda x: x[0])
    list.reverse()
    c_courses = list[0:8]
